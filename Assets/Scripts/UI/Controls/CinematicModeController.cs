...
public class CinematicModeController : MonoBehaviour
{
    ...
    private void SetTarget(string bodyName)
    {
        ...
        if (targetObj != null) selectedTarget = targetObj.transform;
    }

    public void ClearTarget() => selectedTarget = null;
    ...
    private void SetupCinematicCamera()
    {
        ...
        float distanceToPivot = Mathf.Clamp(currentDistance, targetRadius * framingDistanceMultiplier, targetRadius * framingDistanceMultiplier * 3f);
        ...
        Vector3 newCinematicPosition = activePivot.position + (flatDirection * distanceToPivot * Mathf.Cos(pitchInRadians)) + (Vector3.up * distanceToPivot * Mathf.Sin(pitchInRadians));
